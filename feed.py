...
try:
    ...
    feed_pio = android_caps.wd.find_element_by_id("tv_feed_site").text
    ...
    sleep(5)
    pio = android_caps.wd.find_element_by_id("site_name").text
...
try:
    android_caps.wd.find_element_by_id("ad_feed_btn_close").is_displayed()
    android_caps.wd.find_element_by_id("ad_feed_btn_close").click()
    android_caps.wd.find_element_by_android_uiautomator('new UiSelector().text("不感兴趣")').click()
except:
    try:
...
sleep(5)
# <=====附近人筛选=====>
android_caps.wd.find_element_by_id("nearby_feed_filter").click()
print "<=====当前选择项=====>"
genderAll = android_caps.wd.find_element_by_id("filter_radiobutton_genderAll")
genderMale = android_caps.wd.find_element_by_id("filter_icon_radio_genderMale")
genderFemale = android_caps.wd.find_element_by_id("filter_icon_radio_genderFemale")
...
if genderAll.get_attribute("checked") == "true":
    ...
    print "性别: 没有选中性别! "
if android_caps.wd.find_element_by_id("tv_age_range").text == "全部":
    x_num = random.randint(160, 920)
    android_caps.wd.swipe(start_x=969, start_y=960, end_x=x_num, end_y=960, duration=3000)
    sleep(1)
    android_caps.wd.swipe(start_x=160, start_y=960, end_x=x_num, end_y=960, duration=3000)
set_age = android_caps.wd.find_element_by_id("tv_age_range").text
print "年龄:"+set_age
android_caps.wd.find_element_by_id("distance_filter_switch").click()
near = android_caps.wd.find_element_by_id("distance_filter_switch").get_attribute("checked")
if near == "true":
    print "优先看距离近的?===>是"
else:
    print "优先看距离近的?===>否"
sleep(3)
android_caps.wd.find_element_by_id("btn_ok").click()
sleep(5)
# <=====年龄是否符合筛选设置=====>
list_dis = []
for i in range(5):
