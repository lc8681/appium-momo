...
# android_caps.wd.swipe(start_x=700, start_y=770, end_x=700, end_y=2000, duration=500)  下拉刷新
print "< 登录流程start >"
# sleep(5)
...
# check 圈子通知数量
try:
    android_caps.wd.find_element_by_id("new_quanzi_bubble").is_displayed()
    quanzi_bubble = android_caps.wd.find_element_by_id("new_quanzi_bubble").text
    print "$ 圈子通知数:" + quanzi_bubble
    android_caps.wd.find_element_by_id("quanzi_layout").click()   # 点击圈子入口
    sleep(15)
    btn_bubble = android_caps.wd.find_element_by_id("group_btn_bubble").text
    if btn_bubble == quanzi_bubble:
        print "$ 个人帧圈子通知数量与圈子内通知数量相同"
        sleep(5)
        android_caps.wd.find_element_by_id("group_btn_bubble").click()
        sleep(5)
        try:
            android_caps.wd.find_element_by_id("item_layout").is_displayed()
            print "$ 圈子通知展示正确"
    ...
    sleep(15)
# -------------
# android_caps.wd.find_element_by_accessibility_id("AutoQA 圈主 ").click()
# sleep(15)
# ------------
...
try:
    android_caps.wd.find_element_by_accessibility_id("1个人只能创建1个圈子，无法创建").is_displayed()
...
if choice(ran) == comment_pic:
    ...
    android_caps.wd.find_element_by_id("message_btn_sendtext").click()
    sleep(2)
# android_caps.wd.swipe(start_x=534, start_y=1534, end_x=534, end_y=415, duration=None)  # 滚动页面
try:
...
try:
    ...
    if choice(ran) == feed_share:
        ...
        android_caps.wd.find_element_by_accessibility_id("搜索").click()
...
# 加精
print "$ 设置精华..."
android_caps.wd.find_element_by_accessibility_id("more").click()
android_caps.wd.find_element_by_accessibility_id("帖子加精").click()
android_caps.wd.find_element_by_accessibility_id("确定").click()
sleep(10)
android_caps.wd.swipe(start_x=700, start_y=770, end_x=700, end_y=2000, duration=500)
sleep(5)
android_caps.wd.find_element_by_accessibility_id("热门").click()
sleep(3)
android_caps.wd.swipe(start_x=700, start_y=770, end_x=700, end_y=2000, duration=500)
# check群组tab是否有数据
try:
    ...
    print "$ 设置精华失败"
sleep(5)
android_caps.wd.find_element_by_accessibility_id("最新").click()
# 置顶
print "$ 设置置顶..."
android_caps.wd.find_element_by_accessibility_id("more").click()
android_caps.wd.find_element_by_accessibility_id("帖子置顶").click()
android_caps.wd.find_element_by_accessibility_id("确定").click()
sleep(10)
android_caps.wd.swipe(start_x=700, start_y=770, end_x=700, end_y=2000, duration=500)
sleep(5)
try:
    android_caps.wd.find_element_by_accessibility_id("1477971539516-top").is_displayed()
    print "$ 置顶设置成功"
except:
    print "$ 置顶设置失败"
sleep(10)
print "< 操作帖子end,群组tab操作start >"
android_caps.wd.find_element_by_accessibility_id("群组").click()
sleep(3)
# 添加圈子群组
android_caps.wd.find_element_by_accessibility_id("添加群组").click()
sleep(10)
android_caps.wd.find_element_by_class_name("android.widget.EditText").click()
sleep(3)
android_caps.wd.find_element_by_class_name("android.widget.EditText").send_keys("34805293")
sleep(10)
try:
    android_caps.wd.find_element_by_accessibility_id("添加").is_displayed()
    print "$ 搜索群组正确"
except:
    print "$ 没有搜索到群组,测试结束..."
    android_caps.wd.quit()
# check添加
android_caps.wd.find_element_by_accessibility_id("添加").click()
try:
    android_caps.wd.find_element_by_accessibility_id("已添加").is_displayed()
    print "$ 添加群组成功"
except:
    print "$ 添加群组失败..."
...
android_caps.wd.find_element_by_class_name("android.widget.EditText").click()
sleep(3)
android_caps.wd.find_element_by_class_name("android.widget.EditText").clear()
sleep(3)
android_caps.wd.find_element_by_class_name("android.widget.EditText").send_keys("15649776")
sleep(10)
try:
    android_caps.wd.find_element_by_accessibility_id("添加").is_displayed()
    print "$ 搜索群组正确"
...
try:
    android_caps.wd.find_element_by_accessibility_id("加入").is_displayed()
    print "$ 群组tab—未加入群组展示正确"
    # 点击加入按钮
    android_caps.wd.find_element_by_accessibility_id("加入").click()
    sleep(10)
    try:
        android_caps.wd.find_element_by_android_uiautomator('new UiSelector().text("申请加入群组")').is_displayed()
        print "$ 申请加入群组页面进入成功"
        print "$ " + android_caps.wd.find_element_by_id("person_group_information").text
        android_caps.wd.find_element_by_class_name("android.widget.ImageButton").click()
    except:
...
android_caps.wd.find_element_by_id("toolbar_single_menu_id").click()
sleep(15)
# 管理员
android_caps.wd.find_element_by_android_uiautomator('new UiSelector().className("android.view.View").index(9)').click()
manager = android_caps.wd.find_element_by_id("web_title_textview").text
if manager == "圈子管理员":
    print "$ 圈子管理员页面载入正确"
    android_caps.wd.find_element_by_class_name("android.widget.EditText").click()
    sleep(2)
    android_caps.wd.find_element_by_class_name("android.widget.EditText").send_keys("173489501")
    try:
        android_caps.wd.find_element_by_accessibility_id("任命").is_displayed()
        print "$ 管理员搜索结果展示正确"
    except:
        print "$ 管理员搜索结果展示不正确"
else:
...
sleep(2)
# 精华帖
android_caps.wd.find_element_by_accessibility_id("精华帖").click()
sleep(5)
try:
    android_caps.wd.find_element_by_accessibility_id("1477308133728-comment").is_displayed()
    print "$ 精华帖展示正确"
except:
    print "$ 精华帖展示不正确!"
sleep(2)
...
sleep(2)
# 我的圈内等级
android_caps.wd.find_element_by_accessibility_id("我的圈内等级孤独求败").click()
...
if level == "圈子成员等级":
    ...
    sleep(2)
    try:
        android_caps.wd.find_element_by_accessibility_id("选择模板").is_displayed()
        print "$ 成员等级模板弹出正常"
        android_caps.wd.find_element_by_accessibility_id("成就").click()
        sleep(2)
        try:
            android_caps.wd.find_element_by_accessibility_id("选择模板").is_displayed()
            print "$ 成就选择失败!"
...
android_caps.wd.find_element_by_accessibility_id("添加").click()
sleep(15)
friendship = android_caps.wd.find_element_by_id("web_title_textview").text
if friendship == "友情圈子":
    print "$ 友情圈子页面载入正确"
    android_caps.wd.find_element_by_class_name("android.widget.EditText").click()
    sleep(2)
    ...
    try:
        ...
        sleep(3)
        try:
            android_caps.wd.find_element_by_accessibility_id("删除").is_displayed()
            print "$ 添加友情圈子成功"
        except:
            print "$ 添加友情圈子失败!"
    except:
...
sleep(3)
try:
    android_caps.wd.find_element_by_accessibility_id("mmqa").is_displayed()
    print "$ 友情圈子资料页展示正确"
except:
    print "$ 友情圈子资料页展示失败!"
# 删除友情圈子
android_caps.wd.find_element_by_accessibility_id("添加").click()
sleep(10)
android_caps.wd.find_element_by_accessibility_id("删除").click()
sleep(3)
android_caps.wd.find_element_by_class_name("android.widget.ImageButton").click()
sleep(5)
try:
...
sleep(3)
# 编辑圈子
android_caps.wd.find_element_by_accessibility_id(" 编辑圈子").click()
sleep(3)
edit = android_caps.wd.find_element_by_id("web_title_textview").text
if edit == "圈子资料":
    print "$ 编辑圈子页面载入正确"
else:
    print "$ 编辑圈子页面载入异常!"
...
sleep(3)
# 分享圈子
android_caps.wd.find_element_by_accessibility_id(" 分享圈子").click()
sleep(2)
try:
    android_caps.wd.find_element_by_id("alertTitle").is_displayed()
    print "$ 分享框正确弹出"
...
android_caps.wd.find_element_by_accessibility_id("退出圈子").click()
sleep(3)
try:
    android_caps.wd.find_element_by_accessibility_id("提示").is_displayed()
    print "$ 确认框弹出正常"
    sleep(3)
    android_caps.wd.find_element_by_accessibility_id("确定").click()
    sleep(5)
...
android_caps.wd.find_element_by_class_name("android.widget.ImageButton").click()
try:
    android_caps.wd.find_element_by_accessibility_id("AutoQA 圈主 ").is_displayed()
    print "$ 圈子退出失败!..."
except:
    print "$ 圈子退出成功..."
sleep(5)
print "< 退出圈子end,我的帖子start >"
android_caps.wd.find_element_by_accessibility_id("我的帖子").click()
sleep(3)
android_caps.wd.swipe(start_x=534, start_y=1534, end_x=534, end_y=415, duration=None)
sleep(3)
try:
    android_caps.wd.find_element_by_accessibility_id("more").is_displayed()
    android_caps.wd.find_element_by_accessibility_id("1477308133728-comment").is_displayed()
    android_caps.wd.find_element_by_accessibility_id("1479361325382-share").is_displayed()
    print "$ 发布tab各功能元素展示正确"
except:
    print "$ 发布tab元素展示异常!"
sleep(3)
android_caps.wd.find_element_by_accessibility_id("评论").click()
sleep(3)
android_caps.wd.swipe(start_x=534, start_y=1534, end_x=534, end_y=415, duration=None)
sleep(3)
try:
...
# 查看全部
android_caps.wd.find_element_by_accessibility_id("查看全部 ").click()
sleep(10)
try:
    android_caps.wd.find_element_by_accessibility_id("mmqa圈主").is_displayed()
    print "$ 我的圈子页面加载正确"
except:
    print "$ 我的圈子页面加载异常!"
sleep(5)
...
sleep(5)
try:
    android_caps.wd.find_element_by_accessibility_id("最新").is_displayed()
    android_caps.wd.find_element_by_class_name("android.widget.EditText").click()
    sleep(2)
    android_caps.wd.find_element_by_accessibility_id("情感交流").is_displayed()
    print "$ 添加圈子页面数据显示正确"
except:
    print "$ 添加圈子页面数据显示异常!"
sleep(3)
android_caps.wd.find_element_by_accessibility_id("取消").click()
sleep(3)
android_caps.wd.find_element_by_class_name("android.widget.ImageButton").click()
sleep(5)
# 校验最新帖子数据是否展示正常
try:
    android_caps.wd.find_element_by_accessibility_id("最新帖子").is_displayed()
    print "$ 最新帖子数据显示正确"
except:
    print "$ 最新帖子数据显示异常!"
sleep(5)
