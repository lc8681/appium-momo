...
# android_caps.self.swipe(start_x=700, start_y=770, end_x=700, end_y=2000, duration=500)  下拉刷新
print "< 登录流程start >"
# sleep(5)
# try:  # 判断是否处于登录状态,如果登录则直接进入圈子,如果未登录执行登录流程
#     android_caps.self.find_element_by_id("visitor_fast_register").is_displayed()
#     print "$ 帐号未登录,执行登录流程..."
#     android_caps.self.find_element_by_id("maintab_layout_profile").click()   # 点击个人帧
#     sleep(2)
#     android_caps.self.find_element_by_id("visitor_btn_login").click()    # 点击登录按钮
#     sleep(2)
...
#
#     android_caps.self.find_element_by_id("login_et_momoid").send_keys("219530396")  # 输入用户名
#     android_caps.self.find_element_by_id("login_et_pself").click()
#     sleep(1)
#     android_caps.self.find_element_by_id("login_et_pself").send_keys("momo1234")     # 输入密码
#     android_caps.self.find_element_by_id("btn_ok").click()   # 确定登录
#     sleep(10)
#     try:
#         android_caps.self.find_element_by_id("maintab_layout_nearby").is_displayed()
#         print "$ 帐号登录成功..."
#     except:
#         print "$ 账号登录失败"
#
...
# 点击创建圈子按钮
android_caps.self.find_element_by_android_uiautomator('new UiSelector().className("android.view.View").index(3)').click()
try:
...
sleep(15)
# 管理员
android_caps.self.find_element_by_android_uiautomator('new UiSelector().className("android.view.View").index(9)').click()
